...
class PlacesProtocol(asyncio.Protocol):
    ...
    def respond(self, buf):
        ...
        if(message[0] == 'IAMAT'):
            ...
            factory = self.loop.create_connection(lambda: GoogleProtocol(request, self.loop, self),
                                                  host, 'https', ssl=True)
            self.loop.create_task(factory)
        else:
            self.respond_to_invalid(buf)
...
sercor = loop.create_server(lambda: PlacesProtocol(loop), '127.0.0.1', port_of[server_name])
